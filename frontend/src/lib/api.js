...
export const publicApi = {
  ...
  memoryMapMemories: (division, options = {}, fallback) => {
    const safeDivision = requireId(division, "Bangladesh division is required.");
    const api = SummaryApi.memoryMapMemories(safeDivision);
    ...
    return requestOrFallback(
      {
        ...api,
        url: query ? `${api.url}?${query}` : api.url,
      },
      { method: api.method },
      fallback,
    );
