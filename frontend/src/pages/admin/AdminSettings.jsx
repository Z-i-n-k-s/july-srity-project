...
export default function AdminSettings() {
  ...
  const Toggle = ({ name, label, description, icon: Icon }) => <label className="flex cursor-pointer items-start gap-4 rounded-xl border border-white/[0.08] bg-white/[0.025] p-4"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-archive-teal/10 text-archive-teal"><Icon className="h-5 w-5" /></span><span className="flex-1"><span className="block text-sm font-semibold text-white">{label}</span><span className="mt-1 block text-xs leading-5 text-archive-muted">{description}</span></span><input type="checkbox" checked={values[name]} onChange={(e) => update(name, e.target.checked)} className="mt-2 h-5 w-5 accent-[#D79A54]" /></label>;
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="admin-card"><div className="flex items-center gap-3"><ShieldCheck className="h-6 w-6 text-archive-amber" /><h3 className="font-display text-3xl font-semibold">{pick("Review safeguards", "পর্যালোচনা সুরক্ষা")}</h3></div><div className="mt-5 space-y-3"><Toggle name="requireDecisionNote" label={pick("Require decision notes", "সিদ্ধান্ত নোট বাধ্যতামূলক")} description={pick("Reject and information-request actions require an accountable reason.", "প্রত্যাখ্যান ও তথ্য চাওয়ার জন্য জবাবদিহিমূলক কারণ প্রয়োজন।")} icon={FileKey2} /><Toggle name="requirePrivacyCheck" label={pick("Require privacy confirmation", "গোপনীয়তা নিশ্চিতকরণ বাধ্যতামূলক")} description={pick("Reviewers confirm identity, face, voice and metadata protections.", "রিভিউয়ার পরিচয়, মুখ, কণ্ঠ ও মেটাডাটা সুরক্ষা নিশ্চিত করেন।")} icon={LockKeyhole} /><Toggle name="preventOriginalPublish" label={pick("Block original-file publication", "মূল ফাইল প্রকাশ বন্ধ")} description={pick("Only reviewed derivative copies may be public.", "কেবল পর্যালোচিত সংস্করণ প্রকাশ্য হতে পারে।")} icon={ShieldCheck} /></div></section>
        <section className="admin-card"><div className="flex items-center gap-3"><Bell className="h-6 w-6 text-archive-rose" /><h3 className="font-display text-3xl font-semibold">{pick("Notifications", "বিজ্ঞপ্তি")}</h3></div><div className="mt-5 space-y-3"><Toggle name="notifyUrgentSupport" label={pick("Urgent support alerts", "জরুরি সহায়তা সতর্কতা")} description={pick("Notify authorised admins when urgent support cases arrive.", "জরুরি সহায়তা কেস এলে অনুমোদিত অ্যাডমিনকে জানান।")} icon={Bell} /><Toggle name="notifyMissingReport" label={pick("High-priority missing reports", "উচ্চ অগ্রাধিকার নিখোঁজ রিপোর্ট")} description={pick("Surface new reports that require rapid verification.", "দ্রুত যাচাই প্রয়োজন এমন নতুন রিপোর্ট দেখান।")} icon={Bell} /><label className="block rounded-xl border border-white/[0.08] bg-white/[0.025] p-4"><span className="field-label">{pick("Review reminder after", "পর্যালোচনা স্মরণ করাবে")}</span><select value={values.reviewReminderHours} onChange={(e) => update("reviewReminderHours", Number(e.target.value))} className="field-control"><option value={12}>12 hours</option><option value={24}>24 hours</option><option value={48}>48 hours</option></select></label></div></section>
