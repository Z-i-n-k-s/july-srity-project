...
import { CalendarDays, Loader2, MapPin, Play, Search, SlidersHorizontal } from "lucide-react";
...
import { divisionMapMeta, fallbackMapMemories } from "../../data/mapData";

const types = ["All", "Photograph", "Video", "Testimony", "Document", "Story", "Timeline"];

const mapFilterByArchiveType = {
  Photograph: "photo",
  Video: "video",
  Testimony: "testimony",
  Story: "testimony",
...
export default function ArchivePage() {
  ...
  const [total, setTotal] = useState(featuredArchive.length);
  ...
  const selectedDivision = useMemo(() => {
    ...
    return divisionMapMeta.find(
      (division) => division.slug === normalized || division.name.toLowerCase() === normalized,
    ) || null;
  }, [location]);

  const activeMapFilter = mapFilterByArchiveType[type] || "all";
  const requestKey = selectedDivision
    ? `${selectedDivision.slug}:${activeMapFilter}`
    : "all-locations";
  ...
  useEffect(() => {
    ...
    const request = selectedDivision
      ? publicApi.memoryMapMemories(
          selectedDivision.slug,
          { type: activeMapFilter, limit: 24 },
          () => fallbackMapMemories(selectedDivision.slug, activeMapFilter),
        )
      : publicApi.archive(featuredArchive);
    ...
    request
      ...
      .catch((error) => {
        if (!active || version !== requestVersion.current) return;
        setLoadError(error.message || pick("Archive records could not be loaded.", "আর্কাইভ রেকর্ড লোড করা যায়নি।"));
      })
      .finally(() => {
    ...
    return () => { active = false; };
  }, [requestKey, selectedDivision, activeMapFilter, pick]);

  const loadMore = async () => {
    if (!selectedDivision || !cursor || !hasMore || loadingMore) return;
    ...
    try {
      const payload = await publicApi.memoryMapMemories(
        selectedDivision.slug,
        { type: activeMapFilter, limit: 24, cursor },
        () => fallbackMapMemories(selectedDivision.slug, activeMapFilter),
      );
      ...
      if (version === requestVersion.current) {
        setLoadError(error.message || pick("More records could not be loaded.", "আরও রেকর্ড লোড করা যায়নি।"));
      }
    } finally {
    ...
    <>
      ...
      <section className="section-pad">
        <div className="page-shell">
          <div className="grid gap-3 rounded-2xl border border-white/10 bg-white/[0.03] p-4 md:grid-cols-[1fr_auto_auto]">
            ...
            <label>
              ...
              <select value={location} onChange={(e) => setLocation(e.target.value)} className="field-control min-w-44">
                ...
                {divisionMapMeta.map((division) => (
                  <option key={division.slug} value={division.name}>{pick(division.name, division.nameBn)}</option>
                ))}
              </select>
            </label>
          </div>
          <div className="mt-6 flex items-center justify-between text-sm text-archive-muted">
            <span aria-live="polite">
              {loading
                ...
                : selectedDivision
                  ? query
                    ? pick(`${items.length} matching among ${records.length} loaded records`, `লোড হওয়া ${records.length}টি রেকর্ডের মধ্যে ${items.length}টি মিলেছে`)
                    : pick(`${records.length} of ${total} records loaded`, `${total}টির মধ্যে ${records.length}টি রেকর্ড লোড হয়েছে`)
          ...
          {loadError && (
            <div className="mt-5 rounded-xl border border-archive-rose/20 bg-archive-rose/[0.06] px-4 py-3 text-sm text-[#D8B4BC]">
              {loadError}
            </div>
          )}
          ...
          {selectedDivision && hasMore && !loading && (
            <div className="mt-8 flex justify-center">
              <button
                type="button"
                onClick={loadMore}
                disabled={loadingMore}
                className="focus-ring inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-archive-amber/25 bg-archive-amber/[0.08] px-6 font-semibold text-archive-amber transition hover:bg-archive-amber/[0.14] disabled:cursor-wait disabled:opacity-60"
              >
                {loadingMore && <Loader2 className="h-4 w-4 animate-spin" />}
                {loadingMore
                  ? pick("Loading more…", "আরও লোড হচ্ছে…")
                  : pick("Load more records", "আরও রেকর্ড লোড করুন")}
              </button>
            </div>
          )}
