...
const localRooms = useMemo(
  () => [
    ...storage.get(STORAGE_KEYS.supportRooms, []),
    ...demoSupportRooms,
  ],
  [],
);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (Array.isArray(remoteRooms))
        setRooms(
          [...storage.get(STORAGE_KEYS.supportRooms, []), ...remoteRooms].slice(
            0,
            2,
          ),
        );
    ...
    <div>
      ...
      <h1 className="mt-3 font-display text-4xl font-semibold md:text-5xl">
        {pick(
          ...
          `স্বাগতম, ${user?.name?.split(" ")[0] || "ব্যবহারকারী"}।`,
        )}
      </h1>
      <p className="mt-3 text-sm leading-6 text-archive-muted">
        {pick(
          "Continue your submissions, private support cases and saved drafts.",
          "আপনার জমা, ব্যক্তিগত সহায়তা কেস ও সংরক্ষিত খসড়া চালিয়ে যান।",
        )}
      </p>
      ...
      <div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          {
            label: pick("Total submissions", "মোট জমা"),
            value: submissions.length,
            icon: FileText,
          },
          {
            label: pick("Open support rooms", "চলমান সহায়তা কক্ষ"),
            value: rooms.length,
            icon: MessageSquareText,
          },
          {
            label: pick("Saved drafts", "সংরক্ষিত খসড়া"),
            value: drafts.length,
            icon: Clock3,
          },
          {
            label: pick("Verified account", "যাচাইকৃত অ্যাকাউন্ট"),
            value: pick("Active", "সক্রিয়"),
            icon: ShieldCheck,
          },
          ...
          <div key={label} className="surface-card rounded-2xl p-5">
            ...
            <p className="mt-2 text-xs uppercase tracking-[.12em] text-archive-muted">
              {label}
            </p>
          </div>
        ))}
      ...
      <section className="mt-10">
        <div className="flex items-end justify-between gap-4">
          <div>
            <p className="eyebrow">{pick("Quick actions", "দ্রুত কাজ")}</p>
            <h2 className="mt-2 font-display text-3xl font-semibold">
              {pick("What do you need to do?", "আপনি কী করতে চান?")}
            </h2>
          </div>
        </div>
          ...
          {dashboardQuickActions.map(
            ({ title, description, to, icon: Icon }) => (
              <Link
                key={title}
                to={to}
                className="focus-ring group flex gap-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-5 transition hover:-translate-y-0.5 hover:border-archive-amber/30"
              >
                ...
                <ArrowUpRight className="ml-auto h-5 w-5 text-archive-muted transition group-hover:translate-x-1 group-hover:-translate-y-1 group-hover:text-archive-amber" />
              </Link>
            ),
          )}
        </div>
        ...
        <section className="surface-card rounded-2xl p-5 sm:p-6">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-3xl font-semibold">
              {pick("Recent submissions", "সাম্প্রতিক জমা")}
            </h2>
            <Link
              to="/account/submissions"
              className="text-sm font-semibold text-archive-amber"
            >
              {pick("View all", "সব দেখুন")}
            </Link>
          </div>
          ...
          <div className="mt-5 space-y-4">
            {rooms.map((room) => (
              <Link
                ...
                to={`/account/support-rooms/${room.id || room._id}`}
                className="focus-ring flex flex-col gap-3 rounded-xl border border-white/[0.07] p-4 hover:border-archive-teal/25 sm:flex-row sm:items-center sm:justify-between"
              >
                <div>
                  <p className="font-semibold text-white">{room.title}</p>
                  <p className="mt-1 text-xs text-archive-muted">
                    {room.id || room._id} • {room.updatedAt}
                  </p>
                </div>
