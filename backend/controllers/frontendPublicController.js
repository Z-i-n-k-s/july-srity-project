...
function serializeMedia(media) {
  ...
  return {
    id: String(media._id || media.id || url),
    url,
    secureUrl: url,
    name: media.originalName || "Archive media",
...
function serializeArchive(item, options = {}) {
  ...
  const serialized = {
    ...
    location,
    locationId: item.locationId?._id ? String(item.locationId._id) : null,
  ...
  if (includeMedia) {
    const mediaCandidates = [cover, ...(Array.isArray(item.mediaIds) ? item.mediaIds : [])];
    const seen = new Set();
    serialized.media = mediaCandidates
      .map(serializeMedia)
      .filter((media) => {
    ...
    serialized.mediaCount = serialized.media.length;
  }
...
const getArchive = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, {
    ...
    data: serializeArchive(item, { includeMedia: true }),
  });
});
