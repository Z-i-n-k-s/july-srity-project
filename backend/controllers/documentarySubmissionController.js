...
async function validateSubmissionRules(payload) {
  if (payload.submissionType === "CORRECTION") {
    if (!payload.correctionTargetItemId || !payload.correctionReason?.trim()) {
      throw new AppError(
        "Correction submissions require correctionTargetItemId and correctionReason.",
        422,
        "CORRECTION_DETAILS_REQUIRED",
      );
    }
    const target = await DocumentaryItem.exists({
      _id: payload.correctionTargetItemId,
      deletedAt: null,
    });
    if (!target)
      throw new AppError(
        "The correction target archive item was not found.",
        404,
        "CORRECTION_TARGET_NOT_FOUND",
      );
  }
  if (
    ["SOCIAL_MEDIA", "NEWS_SOURCE"].includes(payload.sourceType) &&
    !payload.originalSourceUrl?.trim()
  ) {
    throw new AppError(
      "originalSourceUrl is required for social-media or news-source submissions.",
      422,
      "ORIGINAL_SOURCE_URL_REQUIRED",
    );
  }
  if (
    payload.anonymityPreference === "SHOW_PSEUDONYM" &&
    !payload.pseudonym?.trim()
  ) {
    throw new AppError(
      "A pseudonym is required when the contributor chooses SHOW_PSEUDONYM.",
      422,
      "PSEUDONYM_REQUIRED",
    );
  }
...
async function assertSubmissionReady(payload, req) {
  if (!payload.publicationConsent) {
    throw new AppError(
      "Publication consent is required before this record can be submitted.",
      422,
  ...
  if (
    ...
    throw new AppError(
      "Add testimony text, a description, or at least one media file before submitting.",
      422,
      "SUBMISSION_CONTENT_REQUIRED",
    );
  }
  await validateSubmissionRules(payload);
  await assertMediaAccessible(payload.mediaIds || [], {
    userId: req.userId,
    admin: isAdmin(req),
    allowPublic: true,
  });
...
const create = asyncHandler(async (req, res) => {
  if (req.body.clientDraftId) {
    const existing = await DocumentarySubmission.findOne({
    ...
    if (existing) {
      return sendSuccess(res, {
        message: "This offline testimony draft was already synchronized.",
        data: existing,
      });
    }
  }
  ...
  if (payload.publicationConsent)
    await recordPublicationConsent(req, submission, true);
  ...
  return sendSuccess(res, {
    ...
    message:
      submission.status === "SUBMITTED"
        ? "Testimony submitted for review."
        : "Testimony draft saved successfully.",
    data: submission,
  });
});
...
const listMine = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, {
    message: "Your documentary submissions were retrieved successfully.",
    data: items,
...
const listAdmin = asyncHandler(async (req, res) => {
  ...
  return sendSuccess(res, {
    message: "Documentary submissions retrieved successfully.",
    data: items,
    meta: paginationMeta({ page, limit, total }),
  });
});
...
const getById = asyncHandler(async (req, res) => {
  const submission = await DocumentarySubmission.findOne({
    ...
    .populate(
      "mediaIds",
      "secureUrl url originalName fileType visibility moderationStatus",
    )
  ...
  if (!submission)
    throw new AppError(
      "Documentary submission was not found.",
      404,
      "SUBMISSION_NOT_FOUND",
    );
  requireOwnerOrAdmin(req, submission, ["submittedBy"]);
  return sendSuccess(res, {
    message: "Documentary submission retrieved successfully.",
    data: submission,
  });
});
...
const update = asyncHandler(async (req, res) => {
  ...
  if (
    !isAdmin(req) &&
    !["DRAFT", "NEEDS_INFORMATION"].includes(submission.status)
  ) {
    throw new AppError(
      "Only draft submissions or submissions awaiting information can be edited by the contributor.",
      422,
  ...
  if (Object.prototype.hasOwnProperty.call(updates, "mediaIds")) {
    await assertMediaAccessible(updates.mediaIds || [], {
      userId: req.userId,
      admin: isAdmin(req),
      allowPublic: true,
    });
  ...
  const consentChanged =
    Object.prototype.hasOwnProperty.call(updates, "publicationConsent") &&
  ...
  if (Object.prototype.hasOwnProperty.call(updates, "publicationConsent")) {
    updates.publicationConsentAt = updates.publicationConsent
      ? new Date()
      : null;
  }
  ...
  await submission.save();
  if (consentChanged)
    await recordPublicationConsent(
      req,
      submission,
      Boolean(updates.publicationConsent),
    );
  await writeAudit(req, {
    action: "UPDATE",
    targetType: "DOCUMENTARY_SUBMISSION",
  ...
  return sendSuccess(res, {
    message: "Documentary submission updated successfully.",
    data: submission,
  });
});
...
const assignAdmin = asyncHandler(async (req, res) => {
  ...
  if (!admin)
    throw new AppError(
      "The selected administrator is unavailable.",
      422,
      "INVALID_ADMIN",
    );
  ...
  if (submission.conversationId) {
    await Conversation.findByIdAndUpdate(submission.conversationId, {
    ...
    await ConversationParticipant.findOneAndUpdate(
      {
        ...
        userId: submission.assignedAdminId,
      },
      {
...
const changeStatus = asyncHandler(async (req, res) => {
  ...
  if (!rejectionReason?.trim())
    throw new AppError("A clear rejection reason is required.", 422, "REJECTION_REASON_REQUIRED");
  submission.rejectionReason = rejectionReason.trim();

  // ← PASTE HERE, replacing old findOneAndUpdate
  await DocumentaryItem.findOneAndUpdate(
    {
      $or: [
...
const remove = asyncHandler(async (req, res) => {
  ...
  if (!isAdmin(req) && !["DRAFT", "ARCHIVED"].includes(submission.status)) {
    throw new AppError(
      "A submitted record cannot be deleted by the contributor. Ask an admin to archive it.",
      422,
      "SUBMISSION_DELETE_BLOCKED",
    );
  }
  ...
  await writeAudit(req, {
    ...
    targetType: "DOCUMENTARY_SUBMISSION",
    targetId: submission._id,
    before: submission.toObject(),
  });
  return sendSuccess(res, {
    message: "Documentary submission deleted successfully.",
    data: null,
  });
});
